...
function FamilyMemberList() {
  ...
  return (
    <><div><Navbar /></div><div style={{ display: 'flex', justifyContent: 'center' }}>

      <div className="card EductionCard" id="education">
        {/* Title and Add Member Button */}
        <div className="frame-1190">
          <div className="family-members-title">Family Members</div>
          <div className="add-members-button" onClick={() => console.log('Add member clicked')}>
            <button href="/insertmembers"> ADD MEMBER </button>
          </div>
        </div>

        {/* Member Information Area */}
        <div className="candidate-area" style={{ marginTop: '50px', marginLeft: '19px' }}>
          <div className="container">
            <div className="row">
              {/* Loop through members */}
              {members.map((member) => (
                <div key={member.Id} className="col-lg-12 col-md-12 TableStyle">
                  <div className="InsideCard">
                    <div className="Educationdata">
                      {/* Display Member Details */}
                      <div className="FiledName">
                        Name:
                        <span className="Data">{`${member.FirstName} ${member.MiddleName} ${member.LastName}`}</span>
                      </div>
                      <div className="FiledName">
                        Date Of Birth:
                        <span className="Data">{member.DateOfBirth}</span>
                      </div>
                      <div className="FiledName">
                        Blood Group:
                        <span className="Data">{member.BloodGroup}</span>
                      </div>
                      <div className="FiledName">
                        Marital Status:
                        <span className="Data">{member.MaritalStatus === 0 ? 'Single' : 'Married'}</span>
                      </div>
                      <div className="FiledName">
                        Mobile No:
                        <span className="Data">{`${member.CountryCode}${member.MobileNo}`}</span>
                      </div>
                      <div className="FiledName">
                        Email:
                        <span className="Data">{member.Email}</span>
                      </div>
                      {/* Education and Occupation Buttons */}
                      <button className="education-button" onClick={() => setShowEducationCard(true)}>
                        Education
                      </button>
                      <button className="occupation-button">Occupation</button>
                    </div>
                  </div>
                </div>
              ))}
      ...
      {showEducationCard && (
        <div className="education-card-popup">
          <div className="blur-background"></div>
          <div style={{ position: 'absolute', left: '512px', top: '476px', width: '905px' }}>
            <div className="card EductionCard" id="education">
              <div className="frame-1190">
                <div className="key-skills">Education</div>
                <div className="add-education" onClick={toggleEducationCard}>
                  ADD EDUCATION
                </div>
              </div>
              ...
              <div className="candidate-area" style={{ marginTop: '50px', marginLeft: '19px' }}>
                <div className="container">
                  <div className="row">
                    {/* Loop through educationInfo */}
                    {/* Display Education Card */}
                    {/* Content of the education card */}
                  </div>
                </div>
              </div>
            </div>
